...
const TABLE_NAME = 'ruchi-ai-presence';
  ...
  try {
    ...
    const command = new CreateTableCommand({
      ...
      KeySchema: [
        ...
        { AttributeName: 'sceneId', KeyType: 'RANGE' }
      ],
    ...
    console.log('🎉 Presence table setup complete!');
    console.log('📋 Table details:');
    console.log(`   - Table Name: ${TABLE_NAME}`);
    console.log(`   - Primary Key: userId (HASH) + sceneId (RANGE)`);
    console.log(`   - GSI: sceneId-index`);
    console.log(`   - TTL: ttl attribute for automatic cleanup`);
    
  } catch (error) {
    console.error('❌ Error creating presence table:', error);
    process.exit(1);
  }
}
